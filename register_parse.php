...
$_SESSION['message'] = '';
		...
		if ($_POST['password'] == $_POST['confirmpassword']) {
			...
			$category_user_id = $mysqli->real_escape_string($_POST['category_user_id']);
			$type = $mysqli->real_escape_string($_POST['type']);
			//upload picture to correct directory
			$target_dir = "img/users/";
			$target_file = $target_dir . basename($_FILES["fileToUpload"]["name"]);
			$target_file2 = "http://localhost/job_board/img/users/" . basename($_FILES["fileToUpload"]["name"]);//where the image will be stored
		    ...
		    {
					$sql = "INSERT INTO users (username, password, hash, email, firstname, lastname, category_user_id, type, image) VALUES ('".$username."', '".$password."', '".$hash."', '".$email."', '".$firstname."', '".$lastname."', '".$category_user_id."','".$type."','".$target_file2."')";
